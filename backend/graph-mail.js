...
async function persistRefreshToken(newToken) {
  ...
  try {
    let text = '';
    try { text = await fs.readFile(envPath, 'utf8'); } catch (e) { /* file may not exist */ }
...
async function getAccessToken() {
  ...
  if (resp.data.refresh_token && resp.data.refresh_token !== process.env.REFRESH_TOKEN) {
    if (process.env.NODE_ENV !== 'production') {
      await persistRefreshToken(resp.data.refresh_token);
      console.log('New refresh_token persisted to .env');
    } else {
      console.warn('New refresh_token returned but not persisted in production. Persist to secure store.');
    }
...
export async function sendMail({ to, subject, html }) {
  ...
  try {
    ...
    console.error('Graph sendMail failed:', err.response ? err.response.data : err.message);
    throw err;
  }
}
