...
//   if (!name || !customerId) {
//     return res.status(400).json({ message: "Project name and customerId required" });
//   }

//   const defaultFolders = ["Documents", "Drawings", "Approvals", "Notes"];
...
//        VALUES ($1, $2, $3)
//        RETURNING id, name, customer_id, created_by, created_at`,
...
//     const folderPromises = defaultFolders.map((fname) =>
//       pool.query(
...
//     const folders = results.map((f) => f.rows[0]);
...
export const deleteCompany = async (req, res) => {
  ...
  try {
    ...
    // 4️⃣ Delete ALL users (customer + collaborator)
    if (userIds.length > 0) {
      console.log("🗑 Deleting ALL users (customer + collaborator)...");
      await pool.query(
        `DELETE FROM users
         WHERE id = ANY($1)`,
        [userIds]
...
/* ---------------------------------------------------
   🔟 Add Collaborator to an Existing Company + SEND EMAIL
--------------------------------------------------- */
export const addCollaborator = async (req, res) => {
  ...
  try {
    ...
    // 4️⃣ Create new collaborator user (FIXED ROLE)
    const userRes = await pool.query(
      `INSERT INTO users (name, email, password_hash, role, must_change_password)
    ...
    /* ---------------------------------------------------
       6️⃣ SEND EMAIL TO COLLABORATOR
    --------------------------------------------------- */
/* ---------------------------------------------------
...
Promise.resolve().then(async () => {
  try {
    ...
    console.error("❌ Collaborator background email failed:", emailErr);

    await insertEmailLog({
      customer_id: user.id,
      email: user.email,
      temporary_password: tempPassword,
      subject: "Collaborator Credentials Email",
      body: null,
      status: "error",
      error: emailErr.message || JSON.stringify(emailErr),
    });
...
/* ---------------------------------------------------
   Delete ONLY a collaborator (not the entire company)
--------------------------------------------------- */
export const deleteCollaborator = async (req, res) => {
  const { userId } = req.params;

  try {
    console.log("🗑️ Deleting collaborator:", userId);

    // 1️⃣ Remove relation first
    await pool.query(
      `DELETE FROM user_companies WHERE user_id = $1`,
      [userId]
    );

    // 2️⃣ Delete ONLY collaborators (FIXED ROLE)
    const result = await pool.query(
      `DELETE FROM users WHERE id = $1 AND role = 'collaborator'
       RETURNING id`,
      [userId]
    );
