...
const UploadModal = ({ open, onClose, folderId, projectId, onUploaded }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 🔒 Permission revoked while upload modal was open
      if (status === 403) {
        Swal.fire({
          icon: "error",
          title: "Upload Permission Revoked",
          text: "Your upload access was removed while you were uploading this file. You no longer have permission to upload to this folder.",
          confirmButtonColor: "#ef4444",
        });

        return; // ⛔ stop further handling (no redirect, no crash)
      }

      // 📁 Versioning supported → same title is allowed
      // ❌ NO title-duplicate check here
