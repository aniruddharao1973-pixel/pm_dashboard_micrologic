...
//       if (user.role === "admin" || user.role === "techsales" || user.role === "techsales") {
...
//         }
//       } else {
//         // ⭐ Customer + Collaborator → rely on project.company_name
...
const FoldersPage = () => {
  ...
  if (loading) {
    return (
      <div className="p-4 sm:p-6 md:p-8">
        <p className="text-gray-600 text-base sm:text-lg">Loading folders...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">{projectName} — Folders</h1>
      <p className="text-sm sm:text-base text-gray-600">All folders under this project.</p>

      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
        {folders.length === 0 ? (
          <p className="text-gray-500 text-sm sm:text-base col-span-2 sm:col-span-2 lg:col-span-3">No folders found.</p>
        ) : (
          folders.map((folder) => (
            <FolderCard
              key={folder.id}
