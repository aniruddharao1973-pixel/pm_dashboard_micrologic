...
//   const { getDocumentsByFolder, getDocumentVersions, toggleDownload } = useDocumentsApi();
...
//     if (user.role === "admin" || user.role === "techsales" && project.company_id) {
...
//         ...doc,
//         status: doc.approval_status || doc.status || "submitted",
//       }));
...
//   /** Toggle download allow/deny */
//   // const handleToggleDownload = async (id, value) => {
//   //   try {
//   //     await toggleDownload(id, value);
//   //     loadData();
//   //   } catch (err) {
...
//     customerName && { label: customerName, to: `/projects/${projectId}` },
//     projectName && { label: projectName, to: `/projects/${projectId}/folders` },

//     // ⭐ full chain, but last one without link
//     ...folderChain.map((f, i) => ({
//       label: f.name,
//       to: i === folderChain.length - 1
//         ? null   // last folder → no link
...
//       {/* HEADER */}
// {/* HEADER — Clean, elegant, matches your dashboard */}
// {/* ⭐ Breadcrumb + Upload in one beautiful bar */}
// <div className="
//   flex items-center justify-between 
//   bg-gradient-to-r from-slate-50 to-blue-50 
//   rounded-2xl p-4 border border-gray-300 shadow-sm
// ">
...
//     ...folderChain.map((f, i) => ({
//       label: f.name,
//       to: i === folderChain.length - 1 ? null : `/projects/${projectId}/folders/${f.id}`
//     }))
...
//             transition-all duration-300 relative px-2.5 py-1 rounded-lg
...
//       {/* DOCUMENTS SECTION */}
//       {/* (unchanged, your entire UI stays the same) */}
...
//                 <p className="text-xs text-gray-500 font-medium">{documents.length} files available</p>
//               </div>
//             </div>
//           </div>
//         </div>
...
//                   </div>
//                   <p className="text-gray-500 font-medium text-lg">No documents in this folder</p>
...
//                     <div 
//                       key={doc.id} 
//                       className="group relative bg-gradient-to-br from-white to-gray-50 rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-lg transition-all duration-300 overflow-hidden"
//                     >
...
//       const latest = versions[0]; // Latest version (API usually sorted by created_at DESC)

//       // 2️⃣ Enrich document object with latest version metadata
//       const enrichedFile = {
//         ...doc,
//         version_id: latest.id,
//         status: latest.approval_status || doc.status,
//         uploaded_by: latest.uploaded_by,
//         uploaded_by_role: latest.uploaded_by_role,
//       };

//       // 3️⃣ Open modal with correct version file
//       setViewFile(enrichedFile);

//     } catch (err) {
//       console.error("Error loading latest version:", err);
//       alert("Unable to open document — version lookup failed.");
//     }
//   }}
//   onVersions={() => openVersions(doc)}
//   onDelete={() => {
//     if (user.role === "collaborator") {
//       alert("Collaborators cannot delete documents.");
//       return;
//     }
//     setDeleteFile(doc);
//   }}
...
//                       {/* Allow Download Toggle - Admin Only */}
//                       {/* {user.role === "admin" && (
//                         <div className="px-4 pb-4 pt-2 border-t border-gray-100 bg-gradient-to-br from-blue-50/50 to-purple-50/50">
//                           <label className="flex items-center gap-3 cursor-pointer group/toggle">
//                             <div className="relative">
//                               <input
//                                 type="checkbox"
//                                 checked={doc.can_download}
//                                 onChange={() => handleToggleDownload(doc.id, !doc.can_download)}
//                                 className="sr-only peer"
//                               />
//                               <div className="w-11 h-6 bg-gray-300 rounded-full peer-checked:bg-gradient-to-r peer-checked:from-green-500 peer-checked:to-emerald-500 transition-all duration-300 shadow-inner"></div>
...
//                             </div>
//                             <span className="text-sm font-semibold text-gray-700 group-hover/toggle:text-green-600 transition-colors">
//                               Allow Download
//                             </span>
//                           </label>
//                         </div>
...
//       {/* MODALS (unchanged) */}
//       <UploadModal open={uploadOpen} onClose={() => setUploadOpen(false)} folderId={folderId} projectId={projectId} onUploaded={loadData} />

//       {viewFile && <ViewFileModal file={viewFile} projectId={projectId} folderId={folderId} onClose={() => { setViewFile(null); loadData(); }} />}

//       {versionsFile && <VersionsModal document={versionsFile} versions={versionList} onClose={() => setVersionsFile(null)} />}

//       {renameFile && <RenameModal document={renameFile} onClose={() => setRenameFile(null)} onRename={loadData} />}

//       {deleteFile && <DeleteConfirmModal document={deleteFile} onClose={() => setDeleteFile(null)} onDelete={loadData} />}

//       {notesFile && <NotesModal document={notesFile} onClose={() => setNotesFile(null)} />}
...
//   return (
//     <div className="space-y-2 px-1 sm:px-3 pt-1">

//       {/* HEADER — Responsive breadcrumb and upload button */}
//       <div className="
//         flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3
//         bg-gradient-to-r from-slate-50 to-blue-50 
//         rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-gray-300 shadow-sm
//       ">

//         {/* Breadcrumb LEFT - Responsive with horizontal scroll on mobile */}
//         <div className="w-full sm:w-auto overflow-x-auto">
//           <div className="
//             flex items-center gap-1 sm:gap-1.5 text-sm sm:text-base font-semibold
//             px-3 sm:px-4 py-2 sm:py-2.5 bg-gradient-to-r from-white via-indigo-50/20 to-white 
//             backdrop-blur-sm border border-gray-200/60 rounded-lg sm:rounded-2xl shadow-sm
//             hover:shadow-md hover:border-indigo-200/70 transition-all duration-300
...
//               }))
//             ].filter(Boolean).map((item, index, arr) => (
//               <div key={index} className="flex items-center gap-1 sm:gap-1.5 relative z-10">
//                 {item.to ? (
//                   <Link 
//                     to={item.to} 
//                     className="
//                       text-gray-700 font-semibold hover:text-indigo-600 
//                       transition-all duration-300 relative px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-md sm:rounded-lg
//                       hover:bg-white/80 hover:shadow-sm
//                       after:content-[''] after:absolute after:bottom-0.5
//                       after:left-1.5 sm:after:left-2.5 after:right-1.5 sm:after:right-2.5 after:h-0.5
//                       after:bg-gradient-to-r after:from-indigo-400
//                       after:via-indigo-600 after:to-indigo-400
//                       after:scale-x-0 after:transition-transform
//                       after:duration-300 after:rounded-full
...
//                 ) : (
//                   <span className="
//                     text-indigo-600 font-bold relative px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-md sm:rounded-lg
//                     bg-gradient-to-br from-indigo-50 via-white to-indigo-50
//                     shadow-sm border border-indigo-100/50
//                     hover:shadow-md transition-all duration-300
//                   ">
//                     {item.label}
//                     <span className="
//                       absolute bottom-0.5 left-1.5 sm:left-2.5 right-1.5 sm:right-2.5 h-0.5 
//                       bg-gradient-to-r from-indigo-500 via-indigo-600 to-indigo-500
//                       rounded-full shadow-sm
//                     "></span>
//                   </span>
...
//         {/* Upload button RIGHT - Responsive */}
//         <button
//           onClick={() => setUploadOpen(true)}
//           className="
...
//         <div className="bg-gradient-to-r from-gray-50 to-slate-50 px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
//           <div className="flex items-center justify-between">
//             <div className="flex items-center gap-2 sm:gap-3">
//               <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center shadow-md">
//                 <span className="text-white text-base sm:text-lg">📄</span>
//               </div>
//               <div>
//                 <h2 className="text-lg sm:text-xl font-bold text-gray-800">Documents</h2>
//                 <p className="text-xs text-gray-500 font-medium">{documents.length} files available</p>
//               </div>
//             </div>
//           </div>
...
//           }}
//         >
//           {/* DOCUMENT LIST GRID - Already responsive */}
//           {documents.length === 0 ? (
//             <div className="flex flex-col items-center justify-center py-8 sm:py-16 text-center">
//               <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center mb-4">
//                 <span className="text-4xl sm:text-5xl">📭</span>
//               </div>
//               <p className="text-gray-500 font-medium text-base sm:text-lg">No documents in this folder</p>
//               <p className="text-gray-400 text-xs sm:text-sm mt-1">Upload your first document to get started</p>
//             </div>
//           ) : (
...
//                 <div 
//                   key={doc.id} 
//                   className="group relative bg-gradient-to-br from-white to-gray-50 rounded-lg sm:rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-lg transition-all duration-300 overflow-hidden"
//                 >
//                   {/* Decorative gradient top */}
//                   <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
        ...
        ">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-2.5 md:gap-3">
              ...
              <div>
                <h2 className="text-base sm:text-lg md:text-xl font-bold text-gray-800">Documents</h2>
                <p className="text-xs text-gray-500 font-medium">{documents.length} files available</p>
              </div>
            </div>
          </div>
